...
```java
import com.glodon.jjfab.cloud.util.DateUtils;
import java.util.Date;
import java.util.Objects;
import static com.glodon.jjfab.cloud.util.DateUtils.DATE_FORMAT_YMD;
...
public class DateTypeHandler implements ExportTypeHandler{
    ...
    public String getValue(Object value) throws Exception {
        if (Objects.isNull(value) || !(value instanceof Date)) {
            ...
            return DateUtils.format((Date) value, DATE_FORMAT_YMD);
        }
    }
...
```java
@Data
public class BusinessRegistryDTO {

/**

 * 签约法人
   */
   @ExcelAttribute(name = "签约法人", sort = 1)
   @NotEmpty(groups = {Insert.class, Update.class})
   private String signLegalPerson;

/**

 * 主管单位
   */
   @ExcelAttribute(name = "主管单位", sort = 2)
   @NotEmpty(groups = {Insert.class, Update.class})
 ...
 * 预估签约日期
   */
   @ExcelAttribute(name = "预估签约日期", sort = 3, typeHandler = DateTypeHandler.class)
...
```java
public void exportExcel(HttpServletRequest request, HttpServletResponse response, BusinessRegistryParam param) {
    List<BusinessRegistryDTO> list = baseMapper.queryList(param);
    exportExcel(request, response, "经营台账", list);
}
```
